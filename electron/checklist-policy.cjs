...
function coverageFrom(checkResults = []) {
  const counts = { total: checkResults.length, executed: 0, passed: 0, conflict: 0, missing: 0, missing_candidate: 0, unverifiable: 0, not_applicable: 0, skipped: 0 };
  for (const check of checkResults) {
    const status = String(check?.status || "skipped");
    const key = status === "pass" ? "passed" : status;
    if (["passed", "conflict", "missing", "missing_candidate", "unverifiable", "not_applicable", "skipped"].includes(key)) counts[key] += 1;
    else counts.skipped += 1;
    if (["pass", "conflict", "missing", "missing_candidate"].includes(status)) counts.executed += 1;
  }
  return counts;
}
