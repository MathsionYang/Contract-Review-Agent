...
本文档定义审核工作区对话审核区的产品交互、上下文管理、模型调用、局部审查和企业记忆设计。当前界面将合同原文放在左侧完整区域，合同文件信息、执行配置和任务进度压缩放在合同原文标题上方；右侧风险清单与对话审核垂直分布，风险清单约占 40%，对话审核约占 60%。
...
1. 在审核工作区右侧提供连续对话入口，让用户可以使用自然语言提出审查问题。
...
聊天区放置在审核工作区右侧下方并占据约 60% 空间；合同原文独立占据左侧完整区域，文件上下文信息收纳在原文标题上方，风险清单位于右侧上方约 40%，风险详情仍通过气泡查看。
...
1. 用户可以在审核工作区右侧选择已配置的 `analysis` 模型并发送自然语言问题。
