...
本文档是下一阶段设计方案，不代表当前代码已经完成聊天审查能力。当前 P0 已具备合同解析、规则执行、知识检索、模型网关、风险归一化、局部审查承载、本地持久化和 Validator 导出门禁；本方案将在这些能力之上增加对话入口和上下文编排。
...
- [ ] 在合同原文下方增加聊天区和消息列表。
- [ ] 增加已启用 `analysis` 模型选择，并显示当前执行模型。
- [ ] 新增 `review:chat` IPC 和 `review-chat.cjs`。
- [ ] 实现 ChatSession、ChatMessage 和 ContextSnapshot 本地持久化。
- [ ] 实现普通审查建议和结构化 JSON 输出校验。
- [ ] 处理超时、重试、模型不可用和上下文超限。

### P1-B：风险与局部审查

- [ ] 支持聊天结果生成 `needs_verification` 风险候选。
- [ ] 将风险候选加入当前风险清单，并联动原文和风险详情气泡。
- [ ] 支持聊天触发局部审查，复用选区锚点和上下文扩圈。
- [ ] 增加“查看风险”“加入风险清单”“重新审查此选区”操作。
- [ ] 确保聊天生成结果经过现有人工复核和 Validator 门禁。

### P1-C：企业记忆闭环

- [ ] 实现 L0-L6 记忆召回过滤和排序。
- [ ] 在聊天结果中显示记忆来源、作用域、置信度和有效期。
- [ ] 增加 `memory_candidate` 展示、编辑、确认和放弃。
- [ ] 增加敏感信息扫描、冲突检测和记忆版本。
- [ ] 写入 `MEMORY_CANDIDATE_CREATED`、`MEMORY_CONFIRMED`、`MEMORY_DISMISSED` 审计记录。

### P2：增强能力

- [ ] 流式回复和可取消模型请求。
- [ ] 会话摘要自动压缩和长会话恢复。
- [ ] MCP Tool 与聊天意图的统一工具调用协议。
- [ ] Skill 参与聊天任务的权限、沙箱和执行记录。
...
下一步实现应先落地 P1-A 的上下文快照和普通建议，再接入风险候选与局部审查，最后实现企业记忆候选确认。这样可以让每一步都保持可回溯、可降级和可验证。
