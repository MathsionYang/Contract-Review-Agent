...
本文档只描述当前代码真实具备的能力。当前版本已完成 P0“本地真实审查主链路”，并实现 P1 对话审核、上下文快照、聊天风险/局部审查、企业记忆候选闭环以及 P2 的流式取消、长会话摘要、统一工具协议和本地权限策略。OCR 引擎、真实 MCP/Skill 执行器、账号体系、病毒扫描和多用户协作仍属于后续阶段，不在本阶段宣称完成。
