...
下一阶段已经确定为 M3“核心审查闭环可验收化与可靠性加固”。M3 以当前 `review.config.execution`、`review.task.execution`、`review.chat_sessions`、`ContextSnapshot`、文档解析结果和风险 Schema 为接口，优先完成契约收口、状态机、Checkpoint/幂等恢复、人工风险编辑、选区重新审查、混合记忆检索和 Golden Set 回归。M3 纳入最小可验证的自然语言混合召回链路；大规模向量服务、跨组织记忆治理和在线扩缩容顺延到 M7。OCR、真实 MCP/Skill 执行器、多用户治理和带批注合同副本仍分别顺延到 M4-M6。任何缺少原文、条款或模型结构化依据的结果仍必须保持待核验状态。
