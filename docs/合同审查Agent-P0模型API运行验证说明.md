...
| 最大输出 Token | 例如 `4096` | 通过 `max_tokens` 发送给兼容接口 |
...
`DeepSeek` 只能填写在“服务商”字段，不能作为“模型标识”。模型标识填写 `DeepSeek` 会被服务端判定为不存在并返回 HTTP 400。当前网关也会在请求前拦截该错误，并直接提示使用 `deepseek-chat`；其他 400 错误会显示服务商返回的脱敏原因。
...
2. 点击“校验配置”，确认通过。
3. 点击启用模型。
4. 在审查工作区重新发送对话，或重新执行审查。

如果仍然失败，请记录界面显示的 HTTP 状态和脱敏错误消息，不要记录 API Key、完整 Authorization 请求头或本机凭据文件内容。
